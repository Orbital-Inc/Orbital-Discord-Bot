...
[RequireModerator]
public class AddPermissions(DatabaseContext database) : InteractionModuleBase<ShardedInteractionContext>
{
    ...
    public async Task ExecuteCommand(IUser user, IChannel channel)
    {
        ...
        {
            _ = await logChannel.SendEmbedAsync("Added View Permissions", $"User: {user.Username} - {user.Mention}\nAdded By: {Context.Interaction.User.Mention}", $"{user.Id}", user.GetAvatarUrl());
        }
    }
}